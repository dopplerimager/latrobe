...
pro ncdf_image_strip, fname, temp_dir, strip_name

    finfo    = mc_fileparse(fname)

    sdi3k_read_netcdf_data, fname, metadata=mm, /close
    dt = convert_js(mm.start_time)

	split = strsplit(fname, '.', /extract)
	extn = split[n_elements(split)-1]
	date_str = string(dt.year, f='(i04)') + string(dt.month, f='(i02)') + string(dt.day, f='(i02)')
	destname = temp_dir + finfo.name_only + '_stripped_' + date_str + '.' + extn
...
pro sdi3k_ncdf_filestrip, data_dir, temp_dir, archive_dir, stripped_dir, filter=filter

	if not(keyword_set(filter)) then filter = ['*.pf', '*.nc', '*.sky', '*.las']

	flis = file_search(data_dir + filter, count = nfiles)

	for j = 0, nfiles - 1 do begin

	    ncdf_image_strip, flis[j], temp_dir, strip_name

		;\\ Move the stripped file from the temp directory to the stripped directory
	    file_move, strip_name, stripped_dir + file_basename(strip_name)

		;\\ Move the original file from the data directory to the archive directory
	    file_move, flis[j], archive_dir + file_basename(flis[j])
